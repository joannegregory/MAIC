...
The following packages are required to run these analyses:
...
library(boot, warn.conflicts = FALSE, quietly=TRUE)
library(survminer, warn.conflicts = FALSE, quietly=TRUE) # required for analyses with time to event data
library(survival, warn.conflicts = FALSE, quietly=TRUE) # required for analyses with time to event data
library(MAIC)
#library(haven, warn.conflicts = FALSE, quietly=TRUE) # only required to read in data saved as a '.sas7bdat' file or other 
```
...
data_path <- file.path(base_dir,'2 Exploratory/Simulated data sets')
```
...
comparator_surv <- read.csv(file.path(data_path,"pseudo_IPD.csv"))
...
# baseline_analysis_data <- est_weights$analysis_data %>% 
...
## Summarizing relative treatment effect
