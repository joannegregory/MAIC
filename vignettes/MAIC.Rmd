...
adrs <- adrs %>% # Response data
  ...
  dplyr::select(USUBJID, ARM, response=AVAL) 
...
* Bias-corrected and accelerated (BCa) CIs 
    ...
    * The BCa method also takes percentiles but they are not necessarily the 2.5th and 97.5th percentiles (the choice of percentiles depends on an acceleration parameter [estimated thorugh jackknife resampling] and a bias correction factor [proportion of bootstrap estimates less than the original estimator])

## Example for survival data
### Kaplan-Meier plot

To visualise the effect of the weighting compared to the unadjusted data, it is
...
approach is more appropriate than the percentile approch.
...
When deriving the weighted OR using the GLM, the warnings have been supressed,
