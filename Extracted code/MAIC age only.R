...
target_pop_standard <- target_pop %>%
  ...
  dplyr::transmute(N, Treatment, AGE)
...
baseline_summary$Comparator <- transmute(target_pop_standard, AGE)
...
baseline_summary <- dplyr::bind_rows(baseline_summary) %>%
  ...
  transmute(ARM = as.character(trt), AGE)
